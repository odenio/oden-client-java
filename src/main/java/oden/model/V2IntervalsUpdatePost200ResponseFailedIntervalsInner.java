  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!V2IntervalsUpdatePost200ResponseFailedIntervalsInner.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in V2IntervalsUpdatePost200ResponseFailedIntervalsInner is not found in the empty JSON string", V2IntervalsUpdatePost200ResponseFailedIntervalsInner.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!V2IntervalsUpdatePost200ResponseFailedIntervalsInner.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The field `%s` in the JSON string is not defined in the `V2IntervalsUpdatePost200ResponseFailedIntervalsInner` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
