...
public class OQLQuery {
  ...
  public OQLQuery query(@javax.annotation.Nonnull String query) {
    this.query = query;
    return this;
  }
  ...
  public boolean equals(Object o) {
    ...
    OQLQuery oqLQuery = (OQLQuery) o;
    return Objects.equals(this.query, oqLQuery.query);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to OQLQuery
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!OQLQuery.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in OQLQuery is not found in the empty JSON string", OQLQuery.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!OQLQuery.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The field `%s` in the JSON string is not defined in the `OQLQuery` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : OQLQuery.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!OQLQuery.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'OQLQuery' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<OQLQuery> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(OQLQuery.class));

       return (TypeAdapter<T>) new TypeAdapter<OQLQuery>() {
           @Override
           public void write(JsonWriter out, OQLQuery value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public OQLQuery read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of OQLQuery given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of OQLQuery
   * @throws IOException if the JSON string is invalid with respect to OQLQuery
   */
  public static OQLQuery fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, OQLQuery.class);
  }

  /**
   * Convert an instance of OQLQuery to an JSON string
   *
   * @return JSON string
   */
